...
mappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
  ...
  for (;;) {
    ...
    // SiFive U74 (and other hardware walkers) may raise a page fault rather
    // than hardware-updating A/D bits. Publish accessed on every leaf and
    // dirty on writable leaves so the very first instruction/data access
    // after satp is enabled cannot fault recursively with stvec pointing at
    // an equally unaccessed text page.
    *pte = PA2PTE(pa) | perm | PTE_A | ((perm & PTE_W) ? PTE_D : 0) | PTE_V;
    if (a == last)
      break;
    a += PGSIZE;
    pa += PGSIZE;
