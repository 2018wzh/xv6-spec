...
- `spec lint`、`verify`、`report` 和 `submit` 必须保留确定性和可追溯证据；错误不得被静默吞掉。
- Agent 的 detached worktree 只提供 Git 回滚能力，不是安全沙箱。Agent 可以继承宿主网络、凭据和当前用户权限。
- 修改规格、Makefile、公开验证脚本或文档后，先运行 `vos --project-root examples/xv6-spec spec lint`，再按可用环境运行增量构建和验证。规格由学生手写；`vos agent review` 只能提出建议，不能改写规格。
