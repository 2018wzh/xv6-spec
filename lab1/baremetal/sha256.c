...
// Freestanding helpers (no libc) used by the digest computation.
void *vos_memcpy(void *dst, const void *src, size_t n);
void *vos_memset(void *dst, int c, size_t n);
...
void vos_sha256(const uint8_t *msg, size_t len, uint8_t out[32]) {
    ...
    uint8_t block[64];
    size_t rem = len - full;
    vos_memcpy(block, msg + full, rem);
    block[rem] = 0x80;
    if (rem < 56) {
        vos_memset(block + rem + 1, 0, 56 - rem - 1);
        be64put(block + 56, (uint64_t)len * 8);
        sha256_block(h, block);
    } else {
        vos_memset(block + rem + 1, 0, 64 - rem - 1);
        sha256_block(h, block);
        uint8_t pad[64];
        vos_memset(pad, 0, 64);
