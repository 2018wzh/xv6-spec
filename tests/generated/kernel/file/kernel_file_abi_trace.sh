#!/usr/bin/env sh
# kernel/file bounded trace/oracle target (kernel_file_abi_trace).
#
...
# oracle requires that the file module is linked and initialized in boot
# order (fileinit before process initialization), the kernel reaches the
# scheduler with exactly one canonical XV6_BOOT_OK banner, and no panic text
...
if [ "$status" -ne 0 ] && [ "$status" -ne 124 ]; then
  echo "kernel_file_abi_trace: boot failed (status $status)" >&2
  cat "$output" >&2
  exit 1
fi
...
if grep -q 'panic' "$output"; then
  echo "kernel_file_abi_trace: unexpected panic text in serial output" >&2
  cat "$output" >&2
  exit 1
fi
...
echo "kernel_file_abi_trace: bounded serial trace matched file-module wiring oracle"
