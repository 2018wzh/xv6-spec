...
qemu_log() {
  ...
  if [[ -n "$success_marker" ]]; then
    for _ in $(seq 1 $((timeout_s * 10))); do
      if grep -q -E "$success_marker" "$out" 2>/dev/null; then
...
case "$case_id" in
  bootstrap_banner_not_null|bootstrap_banner_length_positive|boot_banner_printable)
    grep_source 'XV6_BOOT_OK' kernel/boot.c kernel/main.c
  ...
  vf2_platform_contract)
    ...
    grep -q 'starfive,jh7110-visionfive-2' kernel/platform.c
    grep -q 'VF2_v3.8.2' hardware/visionfive2/sources.lock
    ;;
  vf2_image_tools)
    python3 -m py_compile tools/fetch_vf2_assets.py tools/vf2_image.py \
      tools/vf2_hardware_runner.py
    python3 tools/vf2_image.py --help >"$log"
    grep -q 'pyserial==3.5' requirements-hardware.txt
    ;;
  *)
